...
const RiwayatPeminjaman: React.FC = () => {
  ...
  const stats = useMemo(() => {
    ...
    const baik = data.filter(item => item.kondisi_kembali === 'baik').length;
        ...
        <div className="overflow-x-auto">
          <table className="w-full">
            <thead className="bg-slate-900 text-white">
              <tr>
                <th className="p-4 text-left text-[10px] font-black uppercase tracking-widest">ID</th>
                <th className="p-4 text-left text-[10px] font-black uppercase tracking-widest">Mahasiswa</th>
                <th className="p-4 text-left text-[10px] font-black uppercase tracking-widest">Alat & Tujuan</th>
                <th className="p-4 text-left text-[10px] font-black uppercase tracking-widest">Waktu</th>
              ...
              {loading ? (
                ...
                data.map((item) => (
                  <tr key={item.id} className="hover:bg-slate-50/80 transition-colors">
                    <td className="p-4">
                      <div className="font-bold text-slate-400 text-xs">#{item.id}</div>
                    </td>

                    {/* Mahasiswa */}
                    <td className="p-4">
                      <div className="font-black text-slate-800 uppercase italic leading-tight text-sm">
                        {item.user?.name || 'N/A'}
                      </div>
                    ...
                    {/* Alat & Tujuan - FIX UNTUK MULTIPLE ALAT */}
                    <td className="p-4">
                      <div className="flex flex-col gap-1.5">
                        {item.details && item.details.length > 0 ? (
                          item.details.map((detail, idx) => (
                            <div key={idx} className="flex flex-col border-l-2 border-indigo-100 pl-2">
                              <span className="font-bold text-slate-900 text-xs uppercase">
                                {detail.alat?.nama_alat}
                              </span>
                              {detail.alat?.kode_tag && (
                                <span className="text-[9px] font-black text-indigo-500 uppercase tracking-tighter">
                                  TAG: {detail.alat.kode_tag}
                                </span>
                              )}
                            </div>
                          ))
                        ) : (
                          <span className="text-slate-300 italic text-xs">Tanpa data alat</span>
                        )}
                        <div className="mt-1 text-[10px] text-slate-500 font-bold italic uppercase truncate max-w-[180px]">
                          <i className="bi bi-chat-dots mr-1"></i> "{item.tujuan_penggunaan}"
                        </div>
                      </div>
                    </td>
                    ...
                    <td className="p-4 text-[10px]">
                      <div className="flex flex-col gap-1">
                        ...
                        <span className="font-black text-slate-600 bg-slate-50 px-2 py-0.5 rounded border border-slate-200 w-fit">
                          KEMBALI: {item.waktu_kembali ? formatWaktu(item.waktu_kembali) : '---'}
                        </span>
                      </div>
                    </td>
                    ...
                    <td className="p-4 text-right">
                      <div className="flex flex-col gap-1 items-end">
                        <span className={`px-2 py-1 rounded-full text-[9px] font-black uppercase border ${
                          item.status === 'returned' ? 'bg-slate-100 text-slate-600 border-slate-200' : 'bg-amber-50 text-amber-600 border-amber-200 animate-pulse'
                        }`}>
                          {item.status}
                        </span>
      ...
      {selectedImg && (
        ...
        >
          ...
          <div className="relative max-w-5xl w-full h-full flex items-center justify-center" onClick={(e) => e.stopPropagation()}>
            <img src={selectedImg} className="max-w-full max-h-full object-contain rounded-2xl shadow-2xl border-4 border-white/10" alt="Zoom" />
...
// Sub-komponen (StatCard, Loading, Empty)
